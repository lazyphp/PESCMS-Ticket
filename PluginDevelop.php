...
class PluginDevelop {
    ...
    public function create($name) {
        ...
        $this->createPHPInitFile($name, $initPath['0']);

        $this->createCrontabFile($name, $initPath['0']);
     ...
     * @param $patch 应用目录
     */
    private function createPHPInitFile($name, $patch) {
        $fileContent = [
            'header'  => [
                '<?php',
     ...
     * @return void
     */
    private function createCrontabFile($name, $patch) {
        $fileContent = [
            'header'  => [
                '<?php',
                "namespace Plugin\\{$name};",
        ...
        $fopen = fopen("{$patch}/Crontab.php", 'w');
        fwrite($fopen, implode(PHP_EOL, $fileContent['header']) . PHP_EOL);
        foreach ($fileContent['content'] as $item) {
            fwrite($fopen, PHP_EOL . "    public function {$item}() {}" . PHP_EOL);
        }
