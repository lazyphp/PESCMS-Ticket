<form action="/" class="am-form am-form-inline">
    ...
    <div class="am-margin-bottom-xs">
        ...
        <select name="fix" class="am-form-field am-input-sm am-radius" data-am-selected="{btnSize: 'sm', dropUp: 0}">
            ...
            <option value="0" <?= isset($_GET['fix']) && '0' == $_GET['fix'] ? 'selected="selected"' : '' ?>>否</option>
            <option value="1" <?= isset($_GET['fix']) && '1' == $_GET['fix'] ? 'selected="selected"' : '' ?>>是</option>
        </select>
        ...
        <select name="user" class="am-form-field am-input-sm am-radius" data-am-selected="{btnSize: 'sm', dropUp: 0}">
            ...
            <?php foreach($userList as $key => $value): ?>
                <option value="<?= $value['user_id'] ?>" <?= isset($_GET['user']) && $_GET['user'] == $value['user_id'] ? 'selected="selected"' : '' ?> ><?= $value['user_name'] ?></option>
            <?php endforeach; ?>
        </select>
